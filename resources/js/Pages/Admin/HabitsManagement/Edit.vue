<template>
  <Head title="Edit Habit - Momentum" />

  <AdminLayout 
    activeLink="habits" 
    pageTitle="Edit Habit" 
    pageDescription="Modify existing habit details"
  >
        ...
        <div class="p-6 md:p-8">
          ...
          <form @submit.prevent="submit" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <!-- User Display (Read-only on edit typically, but keeping it editable to match the request, though maybe just disabled) -->
              <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">User</label>
                <div class="w-full px-4 py-2.5 rounded-xl border border-gray-200 bg-gray-100 text-gray-600 font-medium cursor-not-allowed">
              ...
              <div class="md:col-span-2">
                ...
                <input type="text" v-model="form.name" class="w-full px-4 py-2.5 rounded-xl border border-gray-300 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors" required />
...
// Since user selection isn't editable in the controller validation typically, we just show it.
const user = computed(() => {
    return props.users.find(u => u.id === props.habit.user_id) || props.habit.user;
});
...
const submit = () => {
  form.put(admin.habits.update(props.habit.id).url);
};
</script>
